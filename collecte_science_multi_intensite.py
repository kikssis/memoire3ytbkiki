...
def watch_history(driver, history_intensity):
    if len(SCIENCE_URLS) < history_intensity:
        raise ValueError(
            f"Pas assez d'URLs science : {len(SCIENCE_URLS)} disponibles, "
            f"mais {history_intensity} demandées."
        )

    history_urls = SCIENCE_URLS[:history_intensity]
...
def main():
    try:
        for history_intensity in HISTORY_INTENSITIES:
            ...
            print("\n################################")
            print(f"DÉBUT HISTORIQUE {history_intensity} VIDÉO(S)")
            print("################################")

            for session_number in range(1, NB_SESSIONS + 1):
                run_session(session_number, history_intensity)
